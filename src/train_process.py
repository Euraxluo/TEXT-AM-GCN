...
from model import GCN,SFGCN
from utils import rocks_cache,cache_with_callable,print_graph_detail, EarlyStopping, accuracy, macro_f1,loss_dependence,common_loss
...
class TextGCNTrainer:
    def __init__(self,
        ...
        self.knn_adjacency_matrix = knn_adjacency_matrix if knn_adjacency_matrix is not None else adjacency_matrix
        ...
        print("knn_adjacency_matrix",self.knn_adjacency_matrix.shape)
    ...
    def move(self):
        ...
        if self.knn_adjacency_matrix is not None:
            self.knn_adjacency_matrix = self.knn_adjacency_matrix.to(self.device)
    def fit(self):
        """
        ...
        if self.model==GCN:
            ...
            raise Exception("not sup this model",self.model)
    ...
    def train(self):
        ...
        with  trange(self.max_epoch) as epoch_pbar:
            for epoch in epoch_pbar:
                ...
                elif  isinstance(self.model,SFGCN):
                    logits, att, emb1, com1, com2, emb2, emb = self.model.forward(self.features, self.adjacency_matrix,self.knn_adjacency_matrix)
                    loss_class =  F.nll_loss(logits[self.train_lst], self.target[self.train_lst])
                    dim = self.adjacency_matrix.shape[0]
                    loss_dep = (loss_dependence(emb1, com1, dim) + loss_dependence(emb2, com2, dim))/2
...
class TrainProcess:
    def __init__(self,
        ...
        if self.model == SFGCN and k is not None:
            # feature graph
            self.knn_adjacency_matrix = cache_with_callable(f'{self.dataset}_knn_adjacency_matrix',lambda:self.knn_graph_to_tensor(k=k)) # self.knn_adjacency_matrix
        else:
            self.knn_adjacency_matrix = self.adjacency_matrix
    ...
    def train(self):
        ...
        for i, seed in enumerate(random.sample(range(0, 100000), self.times)):
            ...
            trainer = TextGCNTrainer(seed=self.seed,
                                     ...
                                     knn_adjacency_matrix=self.knn_adjacency_matrix
                                     )
            trainer.fit()
            trainer.test()
            del trainer
    ...
    def train_valid_test_split(self):
        ...
        train_data = list()
        test_data = list()
        target_data_path = self.path+f"/datasets/target/{self.dataset}.txt"
    ...
    def get_features(self):
        """获取特征矩阵
        """
        # total_document = len(self.target)
        # graph_nodes = self.graph.number_of_nodes()
        # features = []
        # for d_id in trange(total_document, desc='get_features_tensor...'):
        #     values = []
        #     for w_id in range(graph_nodes-total_document, graph_nodes):
        #         values.append(1 if self.graph.get_edge_data(d_id, w_id) else 0)
        #     features.append(values)
        # self.features = torch.from_numpy(np.array(features, dtype=np.int64))
    ...
    def graph_to_tensor(self):
        ...
        sparse_matrix = nx.to_scipy_sparse_matrix(self.graph,
                                                  ...
                                                  dtype=np.float)
        # graph_data_path = self.path+f"/temp/graph/{self.dataset}.txt"   
        # graph_data = np.genfromtxt(graph_data_path, dtype=np.int32)      
        # sedges = np.array(list(graph_data), dtype=np.int32).reshape(graph_data.shape)
        # sparse_matrix = scipy.sparse.coo_matrix((np.ones(sedges.shape[0]), (sedges[:, 0], sedges[:, 1])), shape=(len(self.target), len(self.target)), dtype=np.float32)             
    ...
    def knn_graph_to_tensor(self,k):
        print(f"==> waiting for knn_graph to scipy_sparse_matrix to torch_sparse_tensor <==")
        target_data_path = self.path+f"/temp/knn/{self.dataset}.txt"
        knn_data = np.genfromtxt(target_data_path, dtype=np.int32)       
        # knn_graph = nx.read_weighted_edgelist(target_data_path, nodetype=int)
        # sparse_matrix = nx.to_scipy_sparse_matrix(knn_graph,
                                                #   nodelist=list(
                                                    #   range(knn_graph.number_of_nodes())),
                                                #   weight='weight',
                                                #   dtype=np.float)
        sedges = np.array(list(knn_data), dtype=np.int32).reshape(knn_data.shape)
        sparse_matrix = scipy.sparse.coo_matrix((np.ones(sedges.shape[0]), (sedges[:, 0], sedges[:, 1])), shape=self.adjacency_matrix.shape, dtype=np.float32)             
        need_normalize_adjacency_matrix = sparse_matrix + sparse_matrix.T.multiply(
            sparse_matrix.T > sparse_matrix) - sparse_matrix.multiply(sparse_matrix.T > sparse_matrix)

        self.knn_adjacency_matrix = self.sparse_mx_to_torch_sparse_tensor(
            self.normalize_adjacency_matrix(need_normalize_adjacency_matrix))
        return self.knn_adjacency_matrix
...
if __name__ == '__main__':
    ...
    TrainProcess("logs", 1, SFGCN,max_epoch=250).train()
